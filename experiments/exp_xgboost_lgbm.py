"""
Experiment: XGBoost + LightGBM Ensemble (Submission v4)

Same feature engineering as supervised RF, but ensembles XGBClassifier and
LGBMClassifier. Probabilities are averaged before top-k selection.
...
# Reuse feature extraction from supervised RF experiment
...
def run_experiment(sample_size: int = 100, random_seed: int = 42):
    ...
    print(f"XGBoost+LightGBM (v4): {summary}")

    save_results("xgboost_lgbm", {
        "description": "Ensemble of XGBClassifier and LGBMClassifier with same features as RF",
